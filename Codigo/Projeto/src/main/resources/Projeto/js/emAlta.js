...
async function searchBooksNYT() {
  try {
    const response = await fetch(
      `https://api.nytimes.com/svc/books/v3/lists/current/hardcover-fiction.json?api-key=${apiKeyNYT}`
    );
    ...
    books.forEach(async (book) => {
      ...
      bookElement
        ...
        .addEventListener("click", () =>
          openModal(title, description, imageUrl, authors)
        );
    ...
    booksContainer.innerHTML =
      '<p class="error-message">Não foi possível carregar os livros no momento. Tente novamente mais tarde.</p>';
  }
}
